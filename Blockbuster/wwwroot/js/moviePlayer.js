...
export function initialize(id, movieId, resumeSeconds, initialRevision, settings) {
    ...
    const progressIntervalMs = settings?.progressIntervalMs ?? 10000, resumeThresholdSeconds = settings?.resumeThresholdSeconds ?? 30;
    ...
    const player = createPlayerController(root, { onReady: video => { if (resumeSeconds >= resumeThresholdSeconds && resumeSeconds < video.duration - 10) video.currentTime = resumeSeconds; }, onPlay: () => { save('play'); clearInterval(timer); timer = setInterval(() => save('progress'), progressIntervalMs); }, onPause: () => { clearInterval(timer); save('pause'); }, onEnded: () => { clearInterval(timer); save('ended'); } });
